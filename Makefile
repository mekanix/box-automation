...
base:
.if !exists($(CBSD_WORKDIR)/jails-data/freebsd-11-base-data)
	...
	@sudo sqlite3 $(CBSD_WORKDIR)/jails-system/$(RANDOM_MAC_STRING)/local.sqlite 'DELETE FROM bhyvedsk WHERE jname != "$(RANDOM_MAC_STRING)"'
	@sudo sqlite3 $(CBSD_WORKDIR)/jails-system/$(RANDOM_MAC_STRING)/local.sqlite 'DELETE FROM bhyvenic WHERE jname != "$(RANDOM_MAC_STRING)"'
	@sudo cbsd bclone old=$(RANDOM_MAC_STRING) new=freebsd-11-base
	@sudo sqlite3 $(CBSD_WORKDIR)/jails-system/freebsd-11-base/local.sqlite 'DELETE FROM bhyvedsk WHERE jname != "freebsd-11-base"'
	@sudo sqlite3 $(CBSD_WORKDIR)/jails-system/freebsd-11-base/local.sqlite 'DELETE FROM bhyvenic WHERE jname != "freebsd-11-base"'
	@sudo cbsd bremove $(RANDOM_MAC_STRING)
.endif
	@rm $(TMP_FILE)

init: base
.if !exists($(CBSD_WORKDIR)/jails-data/$(PROJECT)-data)
	@sudo cbsd bclone old=freebsd-11-base new=$(PROJECT)
	@sudo sqlite3 $(CBSD_WORKDIR)/jails-system/$(PROJECT)/local.sqlite 'DELETE FROM bhyvenic WHERE jname != "$(PROJECT)"'
	@sudo sqlite3 $(CBSD_WORKDIR)/jails-system/$(PROJECT)/local.sqlite 'UPDATE bhyvenic SET nic_hwaddr = "$(RANDOM_MAC)"'
	@sudo sh -c 'echo "dhcp-host=$(RANDOM_MAC),$(PROJECT)" >/usr/local/etc/dnsmasq.d/$(PROJECT).vm.conf'
	@sudo zfs set volsize=$(IMAGE_SIZE) tank$(CBSD_WORKDIR)/bcbsd-$(PROJECT)-dsk1.vhd
